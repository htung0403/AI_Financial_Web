...
const ExpensePieChart = (props: { dateRange?: DateRangeType }) => {
  ...
  const categories = React.useMemo(() => {
    const rawCategories = data?.data?.breakdown || [];
    return rawCategories.map((category: CategoryData) => ({
      name: category.name,
      value: category.value, // Already a number from backend
      percentage: category.percentage,
    }));
  }, [data]);

  const totalSpent = React.useMemo(() => {
    const total = data?.data?.totalSpent || 0;
  ...
  const CustomLegend = () => {
    return (
      <div className="grid grid-cols-1 gap-x-4 gap-y-2 mt-4">
        {categories.map((entry: Category, index: number) => (
